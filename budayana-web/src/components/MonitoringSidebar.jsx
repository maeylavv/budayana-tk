...
export default function MonitoringSidebar({ role }) {
  ...
  const getMenuClass = (path, exact = false) => {
    ...
    return `flex items-center justify-center gap-2 px-6 py-2.5 mx-4 my-2 font-fredoka-one font-regular text-lg rounded-full transition-colors ${isActive
  ...
  return (
    <div className="w-64 bg-[#FEF6DF] h-screen flex flex-col shrink-0 border-r border-[#E8D9C0]">
      ...
      <nav className="grow py-6 flex flex-col gap-1 mt-4">
        ...
        <Link
          to="/"
          className="flex items-center justify-center gap-2 px-6 py-2.5 mx-4 mt-6 font-fredoka-one font-regular text-lg text-[#c53030] hover:bg-[#fed7d7] rounded-full transition-colors"
        >
          <span>Keluar</span>
        </Link>
      </nav>
